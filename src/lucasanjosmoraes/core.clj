(ns lucasanjosmoraes.core
  (:import (io.confluent.ksql.api.client ClientOptions Client KsqlObject ExecuteStatementResult ServerInfo Row TableInfo)
...
(defn parse-row-to-map
  ...
  (let [values     (.getList (.values row))
        ...
        filtered-m (select-keys m
                     ["card_number" "email_address" "n_attempts" "tx_ids" "total_amount"])]
...
(defn create-stream
  ...
  (try
    (let [^ExecuteStatementResult result (-> client
                                           (.executeStatement CREATE_TRANSACTIONS_STREAM_STMT
...
(defn get-rows-to-insert
  []
  (list (insert-row "michael@example.com" "358579699410099" "f88c5ebb-699c-4a7b-b544-45b30681cc39" "2020-04-22T03:19:58" "50.25")
...
(defn insert-data
  [^Client client result-chan rows-to-insert]
  (go
    (try
      (doall
        (for [^KsqlObject row-to-insert rows-to-insert]
          (do
            (-> client
              (.insertInto "TRANSACTIONS" row-to-insert)
              (.get))
            (>!! result-chan true))))
      (catch ExecutionException e
        (print-ex (.getMessage (.getCause e)))
...
(defn async-insert-data
  [^Client client]
  (let [rows-to-insert (get-rows-to-insert)
        qtt-to-insert  (count rows-to-insert)
        result-chan    (chan qtt-to-insert)]
    (do
      (insert-data client result-chan rows-to-insert)
      (wait-n-signals 1 qtt-to-insert result-chan))))

(defn create-table
  [^Client client]
  (try
    (let [^ExecuteStatementResult result (-> client
                                           (.executeStatement CREATE_ANOMALIES_TABLE_STMT
                                             {"auto.offset.reset" "earliest"})
                                           (.get))]
...
(defn exec-pull-query
  ...
  (try
    (let [result (-> client
                   (.executeQuery SELECT_ANOMALIES_STMT)
                   (.get))]
      (if (= 0 (count result))
        (println "Empty result")
        (doall
          (for [^Row row result]
            (parse-row-to-map row)))))
    (catch ExecutionException e
      (print-ex (.getMessage (.getCause e))))))
